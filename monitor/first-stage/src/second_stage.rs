...
use mmu::memory_painter::{
    MemoryRange, MemoryRegion as S2MemoryRegion, MemoryRegionKind as S2MemoryRegionKind,
};
...
use crate::mmu::partitioned_memory_map::{MemoryPartition, PartitionedMemoryMap};
...
pub const SECOND_STAGE_SIZE: usize = (1 << 20) * 1800;
...
pub fn load(
    ...
    smp: &Smp,
    memory_partitions: &PartitionedMemoryMap,
    devices: &Vec<Device>,
) {
    ...
    let manifest_s2_mem_regions: &mut [S2MemoryRegion] = unsafe {
        let len = manifest.raw_mem_regions_slice.len() / mem::size_of::<S2MemoryRegion>();
        slice::from_raw_parts_mut(
            manifest.raw_mem_regions_slice.as_mut_ptr() as *mut S2MemoryRegion,
            len,
        )
    };
    ...
    for boot_mr in memory_partitions.get_boot_memory_regions().iter() {
        ...
        manifest_s2_mem_regions[next_boot_mr_idx] = S2MemoryRegion {
            start: boot_mr.start,
            end: boot_mr.end,
